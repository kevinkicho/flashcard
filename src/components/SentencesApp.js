...
export class SentencesApp {
    constructor() {
        ...
        this.userSentence = []; 
        this.wordBankStatus = []; // Tracks which bank words are used (true/false)
    }
    ...
    prev() { audioService.stop(); this.next(); }
    ...
    loadGame() {
        ...
        if (settings.targetLang === 'ja') {
            // Smart Japanese Tokenizer
            words = textService.tokenizeJapanese(cleanSentence);
        ...
        this.wordBankStatus = new Array(this.shuffledWords.length).fill(false); // false = available
        this.userSentence = []; // Array of { word, bankIndex }
    ...
    handleBankClick(bankIndex) {
        if (this.wordBankStatus[bankIndex]) return; // Already used

        // Add to user sentence
        const wordObj = this.shuffledWords[bankIndex];
        this.userSentence.push({ ...wordObj, bankIndex });
        
        // Mark as used (Invisible)
    ...
    handleUserClick(userIndex) {
        ...
        this.wordBankStatus[item.bankIndex] = false; // Make visible again
    ...
    checkWin() {
        ...
        if (this.userSentence.length === this.shuffledWords.length) {
            ...
            // Loose comparison (ignoring spaces for logic check, though join('') handles mostly)
            if (userStr.replace(/\s/g, '') === targetStr.replace(/\s/g, '')) {
                ...
                if (settings.autoPlay) audioService.speak(this.currentData.cleanSentence, settings.targetLang);
    ...
    render() {
        ...
        // LAYOUT: Split Screen (Quiz Style)
        this.container.innerHTML = `
            ...
            <div class="w-full h-full pt-20 pb-28 px-4 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 grid-rows-[1fr_1fr] md:grid-rows-1 gap-4">
                ...
                <div class="w-full h-full bg-white dark:bg-dark-card rounded-[2rem] shadow-xl dark:shadow-none border-2 border-indigo-100 dark:border-dark-border p-6 flex flex-col relative overflow-hidden">
                    ...
                    <div class="mt-8 text-center">
                        <h2 class="text-xl font-bold text-gray-800 dark:text-white leading-snug">${item.back.sentenceOrigin}</h2>
                    </div>
                    ...
                    <div id="sentence-drop-zone" class="flex-grow mt-6 bg-gray-50 dark:bg-black/40 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-4 flex flex-wrap content-start gap-2 overflow-y-auto transition-all duration-300">
                        ...
                        ${this.userSentence.map((obj, i) => `
                            <button class="user-word px-3 py-2 bg-indigo-600 text-white rounded-xl shadow-md active:scale-95 transition-transform font-bold text-lg ${fontClass}" data-index="${i}">
                                ${obj.word}
                            </button>
                        `).join('')}
                    </div>
                ...
                <div class="w-full h-full bg-gray-100 dark:bg-dark-bg/50 rounded-[2rem] border-2 border-transparent p-2 overflow-y-auto">
                    <div class="flex flex-wrap gap-2 justify-center content-start h-full">
                        ${this.shuffledWords.map((obj, i) => `
                            <button class="bank-word px-4 py-3 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border text-gray-700 dark:text-gray-200 rounded-xl shadow-sm hover:shadow-md active:scale-95 transition-all text-lg font-medium ${fontClass} ${this.wordBankStatus[i] ? 'opacity-0 pointer-events-none' : ''}" data-index="${i}">
                                ${obj.word}
                            </button>
                        `).join('')}
                    </div>
