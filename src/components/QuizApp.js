...
export class QuizApp {
    ...
    handleClick(id, buttonElement) {
        ...
        if (settings.quizClickMode === 'double') {
            // First Click: Selection State
            if (this.selectedId !== id) {
                this.selectedId = id;
                
                // Reset all other buttons to default state
                this.container.querySelectorAll('.quiz-option').forEach(btn => {
                    // Reset styling completely to default
                    btn.className = 'quiz-option relative w-full h-full p-2 bg-white dark:bg-dark-card border-2 border-gray-100 dark:border-dark-border rounded-2xl shadow-sm hover:shadow-md active:scale-[0.98] transition-all flex items-center justify-center group overflow-hidden';
                });
        ...
        // CONFIRMATION (Second Click or Single Click Mode)
    ...
    submitAnswer(selectedId, buttonElement) {
        ...
        if (isCorrect) {
            ...
            // Allow retry (Reset internal state)
            this.selectedId = null;
            this.isAnswered = false; 
            
            // Disable this specific button so they can't click it again
    ...
    render() {
        ...
        // HARD-WIRED GRID LAYOUTS
        // We use specific grid row/col configurations to ensure equal sizing
        let gridClass = '';
        if (choices.length === 2) gridClass = 'grid-cols-1 grid-rows-2';
        else if (choices.length === 3) gridClass = 'grid-cols-1 grid-rows-3'; // Stack 3 evenly
        else gridClass = 'grid-cols-2 grid-rows-2'; // Standard 2x2
        ...
        this.container.innerHTML = `
            ...
            <div class="w-full h-full pt-20 pb-28 px-4 flex flex-col items-center max-w-3xl mx-auto gap-4">
                
                <div id="quiz-question-box" class="flex-[3] w-full bg-white dark:bg-dark-card rounded-[2rem] shadow-xl dark:shadow-none border-2 border-indigo-100 dark:border-dark-border p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300">
                    ...
                    <div class="flex-grow w-full flex items-center justify-center overflow-hidden px-4">
                     ...
                     ${target.front.sub ? `<div class="mt-2 text-indigo-500 dark:text-dark-primary font-bold text-xl">${target.front.sub}</div>` : ''}
                </div>

                <div class="flex-[4] w-full grid ${gridClass} gap-3">
                    ${choices.map(choice => `
                        <button class="quiz-option relative w-full h-full p-2 bg-white dark:bg-dark-card border-2 border-gray-100 dark:border-dark-border rounded-2xl shadow-sm hover:shadow-md active:scale-[0.98] transition-all flex items-center justify-center group overflow-hidden" data-id="${choice.id}">
                            <div class="text-lg font-bold text-gray-700 dark:text-gray-200 leading-tight text-center opacity-0 transition-opacity duration-300 w-full px-2" data-fit="true">
        ...
        // AUTO-PLAY AUDIO (Fixed)
        // We use a slightly longer delay to ensure the DOM is ready and previous sounds are killed
