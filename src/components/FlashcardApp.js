...
export class FlashcardApp {
    constructor() {
        ...
        this.touchStartX = 0;
        this.touchEndX = 0;
        this.isMounted = false; // FIX: Prevent duplicate listeners
    }
    ...
    render() {
        ...
        const list = vocabService.getFlashcardData();
        // FIX: Ensure header is rendered even if no data
        const item = (list && list.length > 0) ? list[this.currentIndex] : { id: '-' };
        const content = (list && list.length > 0) 
            ? `<div id="card-container" class="w-full max-w-md aspect-[3/4] relative">${Card(item, this.isFlipped)}</div>`
            : `<div class="p-10 text-center text-gray-500 dark:text-gray-400">No Flashcards Available.</div>`;
        ...
        this.container.innerHTML = `
            <div class="fixed top-0 left-0 right-0 h-16 z-40 px-4 flex justify-between items-center bg-gray-100/90 dark:bg-dark-bg/90 backdrop-blur-sm">
                <div class="flex items-center"><div class="bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-full pl-1 pr-3 py-1 flex items-center shadow-sm"><span class="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-xs font-bold px-2 py-1 rounded-full mr-2">ID</span><span id="fc-id-display" class="font-mono font-bold text-gray-700 dark:text-white text-sm">${item.id}</span></div></div>
                <div class="flex items-center gap-3">
                    <button id="fc-random-btn" class="w-10 h-10 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl flex items-center justify-center text-indigo-500 dark:text-indigo-400 shadow-sm active:scale-90 transition-transform"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg></button>
            ...
            <div class="fixed bottom-0 left-0 right-0 p-6 z-40 bg-gradient-to-t from-gray-100 via-gray-100 to-transparent dark:from-dark-bg">
                <div class="max-w-md mx-auto flex gap-4">
                    <button id="fc-prev-btn" class="flex-1 h-16 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-3xl shadow-sm active:scale-95 transition-all flex items-center justify-center"><svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7" /></svg></button>
        ...
        if(settingsService.get().autoPlay && list && list.length > 0) this.updateCard(); 
    }
}
