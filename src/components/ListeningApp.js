...
export class ListeningApp {
    ...
    loadGame() {
        ...
        if (window.saveGameHistory) window.saveGameHistory('listening', target.id);
    ...
    async handleChoice(id, el) {
        ...
        if (isCorrect) {
            ...
            // "Royal" Pulse Animation on the button
            el.animate([
                { transform: 'scale(1)' },
    ...
    renderButtonState() {
        ...
        if (this.isPlaying) {
            btn.classList.add('ring-4', 'ring-indigo-300', 'scale-110');
            icon.classList.add('animate-pulse');
        } else {
            btn.classList.remove('ring-4', 'ring-indigo-300', 'scale-110');
    ...
    render() {
        ...
        // Use origin definition or sentence as text
        ...
        this.container.innerHTML = `
            ...
            <div class="w-full h-full pt-20 pb-10 px-6 max-w-lg mx-auto flex flex-col justify-center gap-8">
                ...
                <div class="grid grid-cols-1 gap-3 pb-8">
                    ${choices.map(c => `
                        <button class="choice-btn bg-white dark:bg-dark-card border-2 border-gray-100 dark:border-dark-border p-4 rounded-2xl shadow-sm hover:shadow-md text-lg font-bold text-gray-700 dark:text-white transition-all active:scale-98 text-left" data-id="${c.id}">
                            ${getLabel(c)}
                        </button>
                    `).join('')}
                </div>
        ...
        this.container.querySelectorAll('.choice-btn').forEach(btn => {
            btn.addEventListener('click', (e) => this.handleChoice(parseInt(e.currentTarget.dataset.id), e.currentTarget));
        });
    }
}
