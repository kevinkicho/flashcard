...
export class SentencesApp {
    ...
    loadGame() {
        ...
        // [FIX] Validate Data Availability
        ...
        // [FIX] Handle missing sentences safely
        const targetSentence = (item && item.back && item.back.sentenceTarget) 
            ? item.back.sentenceTarget 
            : (item && item.front && item.front.main) ? item.front.main : "No Sentence";
    ...
    renderEmpty() {
        ...
        this.container.innerHTML = `<div class="flex items-center justify-center h-full"><p class="text-gray-500">No sentences available.</p></div>`;
    }
