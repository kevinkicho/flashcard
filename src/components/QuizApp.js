...
export class QuizApp {
    ...
    next(specificId = null) {
        ...
        this.currentData = quizService.generateQuestion(specificId); this.render();
    }
    prev() { this.next(); }
    ...
    submitAnswer(selectedId, buttonElement) {
        ...
        if (isCorrect) {
            ...
            if (settingsService.get().autoPlay) audioService.speak(this.currentData.target.front.main, settingsService.get().targetLang);
    ...
    render() {
        ...
        // LAYOUT FIX: Use Grid for responsive split view
        // Mobile: 1 col, 2 equal rows. Tablet(md): 2 cols, 1 row.
        this.container.innerHTML = `
            ...
            <div class="w-full h-full pt-20 pb-28 px-4 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 grid-rows-[1fr_1fr] md:grid-rows-1 gap-4">
                ...
                <div id="quiz-question-box" class="w-full h-full bg-white dark:bg-dark-card rounded-[2rem] shadow-xl dark:shadow-none border-2 border-indigo-100 dark:border-dark-border p-6 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300">
                    ...
                    <div class="flex-grow w-full flex items-center justify-center overflow-hidden">
                        <span id="quiz-question-text" class="font-black text-gray-800 dark:text-white leading-none select-none opacity-0 transition-opacity duration-300 ${fontClass}" data-fit="true">${target.front.main}</span>
                    </div>
                ...
                <div class="w-full h-full grid ${gridClass} gap-3">
                    ${choices.map(choice => `
                        <button class="quiz-option relative w-full h-full p-2 bg-white dark:bg-dark-card border-2 border-gray-100 dark:border-dark-border rounded-2xl shadow-sm hover:shadow-md active:scale-[0.98] transition-all flex items-center justify-center group overflow-hidden" data-id="${choice.id}">
                            <div class="text-lg font-bold text-gray-700 dark:text-gray-200 leading-tight text-center opacity-0 transition-opacity duration-300 w-full px-2" data-fit="true">${choice.back.definition}</div>
                        </button>
                    `).join('')}
                </div>
            </div>
        ...
        if(settings.autoPlay) setTimeout(() => audioService.speak(target.front.main, settings.targetLang), 300);
